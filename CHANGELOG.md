...
- Και τα 73 view αρχεία συνδέθηκαν με τον κατάλογο μεταφράσεων: **1850
  σημεία** στατικού ελληνικού κειμένου αντικαταστάθηκαν με lookups
  `t('key', 'ελληνικό fallback')`, ώστε η επιλογή γλώσσας στο `/profile`
  να αλλάζει πραγματικά την εμφάνιση — ανεξάρτητα ανά χρήστη (π.χ.
  διαχειριστής σε Ελληνικά, μέλος ομάδας σε English, ταυτόχρονα).
  Οι ανώνυμες σελίδες (login, δημόσιοι σύνδεσμοι, field hub) εμφανίζονται
  πάντα στα Ελληνικά (γλώσσα-πηγή).
- Νέοι helpers `t()` / `current_language()` — ένα query ανά γλώσσα ανά
  request (όχι ανά string), με πλήρη αλυσίδα fallback: μετάφραση →
  ενσωματωμένο ελληνικό κείμενο → γλώσσα-πηγή → κλειδί. Αν λείψει
  οτιδήποτε από τη βάση, η σελίδα εμφανίζει το αρχικό ελληνικό κείμενο —
  ποτέ κενό ή σφάλμα.
- Η αυτόματη σύνδεση έγινε με νέο εργαλείο (`scripts/wire-translations.php`)
  με αυστηρές δικλείδες ασφαλείας (δομικός έλεγχος «ποτέ διαγραφή ζωντανού
  PHP», έλεγχος ισοζυγίου tags, έλεγχος διαρροής attribute syntax) —
  μηδενική αλλοίωση αρχείων, όλα τα 73 αρχεία lint-clean.
- 220 νέα strings που εντοπίστηκαν κατά τη σύνδεση (κείμενο που προστέθηκε
  στα views μετά την αρχική εξαγωγή) μεταφράστηκαν με πλήρες context και
  προστέθηκαν στον κατάλογο — σύνολο πλέον **2021 strings** με πλήρη
  ελληνο-αγγλική ισοτιμία. Νέο migration
  `044_translation_catalog_new_keys.sql` (idempotent) + ενημερωμένο
  `schema.sql` για νέες εγκαταστάσεις.
- Διορθώθηκε το γνωστό θέμα πληθυντικού στο `views/team/debrief.php`
  («αστέρι»/«αστέρια» ως ξεχωριστά κλειδιά αντί για ελληνική κλίση με
  concatenation — «1 star» / «3 stars» σωστά στα αγγλικά), καθώς
  και 9 ακόμη σημεία όπου μονοσύμβολα ελληνικά τμήματα (το άρθρο «Η»,
  η συντομογραφία ωρών «ω», το ελληνικό ερωτηματικό «;») ήταν hardcoded
  δίπλα σε μεταφρασμένο περιεχόμενο.
- Νέα integration tests: η προεπιλεγμένη γλώσσα αποδίδει Ελληνικά, και η
  αλλαγή προτίμησης σε English αλλάζει αποδεδειγμένα το rendered HTML
  (πλήρης έλεγχος μέσω πραγματικού HTTP).
- Εκτός scope (σκόπιμα, καταγεγραμμένα για μελλοντικό έργο): οι τίτλοι
  σελίδας (`<title>`) που ορίζονται στους controllers, τα ελληνικά
  κείμενα μέσα σε JavaScript strings (confirm dialogs κ.λπ.), και ~40
  strings που περιέχουν λατινικό `;` — όλα παραμένουν στα Ελληνικά
  και στις δύο γλώσσες.

## [0.19.0-beta] — 2026-07-15

### Feature — Πλήρης αγγλική μετάφραση του καταλόγου (1801 strings)

- Και τα 1801 strings του καταλόγου μεταφράσεων (βλ. v0.18.0-beta) έχουν
  τώρα πλήρη, προσεκτική αγγλική μετάφραση — κάθε string διαβάστηκε στο
  πραγματικό view αρχείο του για σωστό context πριν μεταφραστεί (όχι
  μηχανική/τυφλή μετάφραση), με ενιαία ορολογία σε όλη την εφαρμογή
  (Event, Team, Volunteer, Municipality, Mobilization, Playbook, Shift,
  Resource request, κ.λπ.).
- Νέο migration `043_translation_catalog_en_seed.sql` (idempotent,
  `ON DUPLICATE KEY UPDATE`) ώστε η μετάφραση να φτάσει και σε
  υπάρχουσες εγκαταστάσεις μέσω του κανονικού update flow· το
  `schema.sql` ενημερώθηκε ώστε και οι νέες εγκαταστάσεις να ξεκινούν
  με πλήρη δίγλωσσο κατάλογο (1801 Ελληνικά + 1801 English).
- Λίγα strings (~40) διατηρούν σκόπιμα κενό σημείο όπου το πρωτότυπο
  template είχε δυναμική τιμή PHP (π.χ. όνομα δράσης) που αφαιρέθηκε
  κατά την αυτόματη εξαγωγή — θα διαβάζονται φυσικά μόλις η δυναμική
  τιμή ενσωματωθεί ξανά στο μελλοντικό έργο σύνδεσης των views με τον
  κατάλογο. Ένα string (`team/event_show.051`, το ελληνικό άρθρο
  «Ο/Η» πριν από όνομα) μεταφράστηκε σκόπιμα ως κενό, καθώς η αγγλική
  γλώσσα δεν έχει αντίστοιχο άρθρο πριν από κύριο όνομα.
- Σημειώθηκε (χωρίς να διορθωθεί εδώ, εκτός scope): το
  `views/team/debrief.php` κάνει hardcoded ελληνική κλίση πληθυντικού
  («αστέρι»/«αστέρια») απευθείας σε PHP string concatenation αντί μέσω
  του συστήματος μετάφρασης — θα χρειαστεί διόρθωση όταν το view αυτό
  συνδεθεί με τον κατάλογο.
