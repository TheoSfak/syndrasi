<div class="d-flex flex-wrap justify-content-between align-items-center mb-1">
  <h1 class="h3 mb-0">Φορείς</h1>
  <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#newMunicipalityModal">
    <i class="bi bi-plus-lg me-1"></i>Νέος Φορέας
  </button>
</div>
<p class="text-muted">Διαχείριση δήμων, πολιτικής προστασίας, πυροσβεστικής και λιμενικού.</p>

<?php $authorityOptions = $authorityOptions ?? authority_options(); ?>

<div class="card shadow-sm">
  <?php if (!$municipalities): ?>
    <div class="card-body text-muted">Δεν έχουν καταχωρηθεί φορείς.</div>
  <?php else: ?>
    <div class="table-responsive">
      <table class="table table-hover mb-0">
        <thead><tr><th>Φορέας</th><th>Τύπος</th><th>Πόλη</th><th>Επικοινωνία</th><th>Κατάσταση</th><th class="text-end">Ενέργειες</th></tr></thead>
        <tbody>
...
<div class="modal fade" id="newMunicipalityModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="post" action="<?= e(url('/admin/municipalities/store')) ?>" class="modal-content">
      <?= csrf_field() ?>
      <div class="modal-header"><h5 class="modal-title">Νέος Φορέας</h5>
      ...
      <div class="modal-body">
        ...
        <div class="mb-2"><label class="form-label">Πλήρες εμφανιζόμενο όνομα</label><input type="text" name="official_name" class="form-control" placeholder="π.χ. Πυροσβεστική Ηρακλείου"></div>
        <div class="mb-2"><label class="form-label">Σύντομο όνομα</label><input type="text" name="short_name" class="form-control" placeholder="π.χ. Πυρ/κή"></div>
...
<div class="modal fade" id="editModal<?= (int) $m['id'] ?>" tabindex="-1">
  <div class="modal-dialog">
    <form method="post" action="<?= e(url('/admin/municipalities/' . $m['id'] . '/update')) ?>" class="modal-content">
      <?= csrf_field() ?>
      <div class="modal-header"><h5 class="modal-title">Επεξεργασία: <?= e($m['official_name'] ?: $m['name']) ?></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body">
        ...
        <div class="mb-2"><label class="form-label">Πλήρες εμφανιζόμενο όνομα</label><input type="text" name="official_name" class="form-control" value="<?= e($m['official_name'] ?? '') ?>"></div>
        <div class="mb-2"><label class="form-label">Σύντομο όνομα</label><input type="text" name="short_name" class="form-control" value="<?= e($m['short_name'] ?? '') ?>"></div>
