<div class="d-flex flex-wrap justify-content-between align-items-center mb-1 gap-2">
  <h1 class="h3 mb-0">Στατιστικά</h1>
...
<p class="text-muted">Στατιστικά δράσεων και εθελοντικής προσφοράς για το έτος <?= (int) $year ?>.</p>
      ...
      <div class="card-body">
        ...
        <?php else: ?>
          <canvas id="eventsByCategoryChart" height="220"
                  data-labels="<?= e(json_encode(array_column($byCategory, 'category'), JSON_UNESCAPED_UNICODE)) ?>"
                  data-values="<?= e(json_encode(array_map('intval', array_column($byCategory, 'total')))) ?>"></canvas>
        <?php endif; ?>
      </div>
    </div>
  </div>
