...
<div class="container py-4" style="max-width:780px">
  ...
  <div class="card shadow-sm border-0">
    <div class="card-header bg-gradient text-white d-flex align-items-center gap-2"
      ...
      <?php if ($debrief): ?>
        <span class="badge bg-success ms-auto">Ήδη υποβλήθηκε — μπορείτε να επεξεργαστείτε</span>
      <?php endif ?>
    </div>
    ...
    <div class="card-body p-4">
      <form method="post" action="<?= e(url('/team/events/' . $event['id'] . '/debrief')) ?>">
        ...
        <h6 class="text-uppercase text-muted mb-3 fw-semibold" style="letter-spacing:.05em">
          <i class="bi bi-bar-chart-fill me-1 text-primary"></i>Αριθμητικά Στοιχεία
        </h6>
        <div class="row g-3 mb-4">
          <div class="col-sm-4">
            <label class="form-label">Εθελοντές που συμμετείχαν</label>
            <input type="number" name="actual_volunteers" min="0" class="form-control"
                   value="<?= e($d['actual_volunteers'] ?? '') ?>" required>
          </div>
          <div class="col-sm-4">
            <label class="form-label">Συνολικές ώρες εθελοντισμού</label>
            <input type="number" name="volunteer_hours" min="0" step="0.5" class="form-control"
                   value="<?= e($d['volunteer_hours'] ?? '') ?>" required>
          </div>
          <div class="col-sm-4">
            <label class="form-label">Αριθμός συμβάντων</label>
        ...
        <div class="mb-3">
          ...
          <textarea name="what_went_well" rows="3" class="form-control"
                    placeholder="Περιγράψτε τι λειτούργησε καλά…"><?= e($d['what_went_well'] ?? '') ?></textarea>
        </div>
        <div class="mb-3">
          <label class="form-label">Τι μπορεί να βελτιωθεί;</label>
          <textarea name="what_went_wrong" rows="3" class="form-control"
                    placeholder="Περιγράψτε προβλήματα ή περιθώρια βελτίωσης…"><?= e($d['what_went_wrong'] ?? '') ?></textarea>
        </div>
        <div class="mb-4">
          <label class="form-label">Περιγραφή συμβάντων <span class="text-muted">(προαιρετικό)</span></label>
        ...
        <div class="d-flex gap-2">
          ...
          <a href="<?= url('/team/events/' . $event['id']) ?>" class="btn btn-outline-secondary">Ακύρωση</a>
        </div>
      </form>
    </div><!-- card-body -->
  </div><!-- card -->
