...
<div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-3">
  <div>
    <h1 class="h3 mb-1"><i class="bi bi-fire text-danger me-1"></i>Συμβάντα Πυροσβεστικής</h1>
    <p class="text-muted small mb-0">Αυτόματη λήψη από το Πυροσβεστικό Σώμα, με φίλτρα για Κρήτη, Π.Ε. και περιοχή.</p>
  </div>
  <form method="post" action="<?= e(url('/fire-service/sync')) ?>">
    <?= csrf_field() ?>
    <button class="btn btn-danger" type="submit">
      <i class="bi bi-arrow-clockwise me-1"></i>Άμεση ενημέρωση
    </button>
  </form>
</div>
...
<div class="row g-3 mb-3">
    ...
    <div class="card shadow-sm h-100">
      <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between gap-2">
          <div>
            <div class="fw-bold">Κατάσταση λήψης</div>
            <div class="text-muted small">
              <?php if ($latestFetch): ?>
                Τελευταία προσπάθεια: <?= e(gr_datetime($latestFetch['fetched_at'])) ?> ·
                <?php if ((int) $latestFetch['success'] === 1): ?>
                  <span class="text-success fw-semibold"><?= (int) $latestFetch['incidents_found'] ?> συμβάντα</span>
                <?php else: ?>
                  <span class="text-danger fw-semibold">Αποτυχία</span>
                <?php endif; ?>
              <?php else: ?>
                Δεν έχει γίνει ακόμη λήψη. Πατήστε «Άμεση ενημέρωση».
              <?php endif; ?>
            </div>
          </div>
          <a class="btn btn-sm btn-outline-secondary" href="<?= e($sourceUrl) ?>" target="_blank" rel="noopener">
            Πηγή <i class="bi bi-box-arrow-up-right"></i>
          </a>
        </div>
  ...
  <div class="col-lg-5">
    <div class="card shadow-sm h-100">
      <div class="card-body">
        <div class="fw-bold mb-2">Cron κάθε 5 λεπτά</div>
...
<form method="get" action="<?= e(url('/fire-service')) ?>" class="card shadow-sm mb-3">
  <div class="card-body row g-3 align-items-end">
    <div class="col-md-3">
      <label class="form-label small fw-semibold">Περιφέρεια</label>
    ...
    <div class="col-md-3">
      <label class="form-label small fw-semibold">Νομός / Π.Ε.</label>
    ...
    <div class="col-md-2">
      <label class="form-label small fw-semibold">Κατηγορία</label>
    ...
    <div class="col-md-2">
      ...
      <input type="search" name="q" class="form-control" value="<?= e($filters['q'] ?? '') ?>" placeholder="π.χ. Ηράκλειο">
    </div>
    <div class="col-md-8">
      <div class="form-check">
        <input class="form-check-input" type="checkbox" name="history" value="1" id="historyChk" <?= ($filters['current'] ?? '1') === '0' ? 'checked' : '' ?>>
        <label class="form-check-label" for="historyChk">Εμφάνιση και ιστορικού 7 ημερών, όχι μόνο τρέχον snapshot</label>
      </div>
    </div>
    <div class="col-md-4 text-md-end">
      <button class="btn btn-primary" type="submit"><i class="bi bi-funnel me-1"></i>Φιλτράρισμα</button>
      <a class="btn btn-outline-secondary" href="<?= e(url('/fire-service')) ?>">Default Κρήτης</a>
    </div>
  </div>
</form>
...
<?php if (!$incidents): ?>
  <div class="card shadow-sm">
    <div class="card-body text-center text-muted py-5">
      ...
      Δεν βρέθηκαν συμβάντα με τα τρέχοντα φίλτρα.
    </div>
  </div>
<?php else: ?>
  <div class="card shadow-sm">
          ...
          <?php foreach ($incidents as $i): ?>
            <tr class="<?= (int) $i['is_current'] === 1 ? '' : 'table-light text-muted' ?>">
              ...
              <td>
                ...
                <?php if ((int) $i['is_current'] !== 1): ?><span class="badge text-bg-secondary">Ιστορικό</span><?php endif; ?>
              </td>
