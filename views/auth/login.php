...
<title><?= e(t('auth/login.001', 'Σύνδεση | SynDrasi')) ?></title>
  ...
  <div class="card login-card shadow-lg">
    <div class="card-body p-4 p-md-5">
      <div class="text-center mb-4">
        ...
        <p class="text-muted mb-0"><?= e(t('auth/login.002', 'Πλατφόρμα συντονισμού δράσεων')) ?><br><?= e(t('auth/login.003', 'και εθελοντικών ομάδων')) ?></p>
      </div>
      ...
      <form method="post" action="<?= e(url('/login')) ?>">
        ...
        <div class="mb-4">
          <label class="form-label" for="password"><?= e(t('auth/login.004', 'Κωδικός πρόσβασης')) ?></label>
          <input type="password" class="form-control form-control-lg" id="password" name="password" required>
        </div>
        <button type="submit" class="btn btn-lg w-100 text-white" style="background:#0f766e">
          <i class="bi bi-box-arrow-in-right me-1"></i> <?= e(t('auth/login.005', 'Σύνδεση')) ?>
        </button>
